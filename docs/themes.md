...
helpers, and the optional config + composer pieces. The bundled
`basic` theme is the canonical reference.
...
$provider = App::container()->get(SubscriberListenerProvider::class);

$provider->subscribe(ItemUpdated::class, function (ItemUpdated $event): void {
    // e.g. ping an analytics endpoint when a page is edited.
});
```

`ItemCreated`, `ItemUpdated` and `ItemDeleted` are the three
item-lifecycle events; `CategoryCreated/Updated/Deleted` and
`FieldCreated/Updated/Deleted` round out the set. See
[iManager `docs/api/domain.md`](https://github.com/bigin/imanager/blob/main/docs/api/domain.md#domain-events)
